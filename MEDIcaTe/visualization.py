...
def stack_tiled_numpy(final_path,ct_tile,pet_tile,labels_tile,cmap_col,norm):
    ...
    axs.imshow(labels_tile, cmap=cmap_col,norm=norm)
...
def visualization(final_path,ct_path=None,pet_path=None,
    ...
    if ct_path is not None:
        if type(ct_path) is str:#check datatype
            ct_nii = nib.load(ct_path)
            ct_numpy = ct_nii.get_fdata()
            [X,Y,Z]=ct_numpy.shape 
        else:
            raise Exception('ct_path must be of type str')
    if pet_path is not None:
        if type(pet_path) is str:#check datatype
            pet_nii = nib.load(pet_path)
            pet_numpy = pet_nii.get_fdata()
            [X,Y,Z]=pet_numpy.shape
        else:
            raise Exception('pet_path must be of type str')
    if l1_path is not None:  
        if type(l1_path) is str:#check datatype
            L1_nii = nib.load(l1_path)
            L1_numpy = L1_nii.get_fdata()
            [X,Y,Z]=L1_numpy.shape 
        else:
            raise Exception('l1_path must be of type str')
    if l2_path is not None:
        if type(l2_path) is str:#check datatype
            L2_nii = nib.load(l2_path)
            L2_numpy = L2_nii.get_fdata()
            [X,Y,Z]=L2_numpy.shape 
        else:
            raise Exception('l2_path must be of type str')
    if l3_path is not None:#check datatype
        if type(l3_path) is str:
            L3_nii = nib.load(l3_path)
            L3_numpy = L3_nii.get_fdata()
            [X,Y,Z]=L3_numpy.shape 
        else:
            raise Exception('l3_path must be of type str')
    ...
    #check datatype
    if ((type(l1_name) or type(l2_name) or type(l3_name)) is not str):
        raise Exception("label names, that is l1_name, l2_name and l3_name, must be of type str")
    if type(slices) is not list:
        raise Exception("slices must be of type list")
    if type(outline) is not bool:
        raise Exception("outline must be of type bool")
    if type(cropping) is not bool:
        raise Exception("cropping must be of type bool")
    if type(all_slices) is not bool:
        raise Exception("all_slices must be of type bool")
    if type(incl_info) is not bool:
        raise Exception("incl_info must be of type bool")
    if type(verbose) is not bool:
        raise Exception("verbose must be of type bool")
...
def Entry_Point():
    ...
    parser.add_argument( '-ct_path',     type=str,default=None,   help='Input path to ct nii-file')
    parser.add_argument( '-pet_path',    type=str,default=None,   help='Input path to pet nii-file')
    parser.add_argument( '-l1_path',     type=str,default=None,   help='Input path to label nii-file')
    parser.add_argument( '-l1_name',     type=str,default='L1',   help='legend name for label 1.______________________________________________________(default: L1)')
    parser.add_argument( '-l2_path',     type=str,default=None,   help='Input path to label nii-file')
    parser.add_argument( '-l2_name',     type=str,default='L2',   help='legend name for label 2.______________________________________________________(default: L2)')
    parser.add_argument( '-l3_path',     type=str,default=None,   help='Input path to label nii-file')
    parser.add_argument( '-l3_name',     type=str,default='L3',   help='legend name for label 3.______________________________________________________(default: L3)')
    parser.add_argument( '-slices',      type=list,default=None,  help='Input list with int values of slices to be included.__________________________(default: all slices in nii is used)')
    ...
    visualization(args.final_path,args.ct_path,args.pet_path,
                    args.l1_path,args.l1_name,
...
if __name__ == '__main__':
    ...
    final_path=f'/home/fmik/data/figs/HNC03_017_refmd'
    ct=f'/home/fmik/data/t_l64_doctors/t_l64/images/HNC03_017_0000.nii.gz' #0000
    pet=f'/home/fmik/data/t_l64_doctors/t_l64/images/HNC03_017_0001.nii.gz' #0001
    l1=f'/home/fmik/data/t_l64_doctors/t_l64/labels/md/HNC03_017_md.nii.gz' #md
    l2=f'/home/fmik/data/t_l64_doctors/t_l64/labels/ref/HNC03_017_ref.nii.gz' #ref
    l3=None#f'/home/fmik/data/t_l64_clinical/labels/HNC03_017.nii.gz' #clinical
    z=None#list(range(4, 7))
    visualization(final_path,ct_path=ct,pet_path=pet,
                  l1_path=l1, l1_name='md',
                  l2_path=l2, l2_name='ref',
                  l3_path=l3, l3_name='clinic',

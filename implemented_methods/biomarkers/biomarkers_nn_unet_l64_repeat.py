...
if __name__ == '__main__':
    path_to_pet = '/homes/kovacs/project_data/hnc-auto-contouring/hnc_rigs_all/data_nifty/t_l64/images'
    #label_niis_ref = '/homes/kovacs/project_data/hnc-auto-contouring/hnc_rigs_all/data_nifty/t_l64/labels/labels_doctor_study/ref_renamed'
    label_niis_pred = '/homes/kovacs/project_data/hnc-auto-contouring/hnc_rigs_all/data_nifty/t_l64/predictions_nnunet/labels_nn_unet_ensemble_final'
    label_niis_pred_r = '/homes/kovacs/project_data/hnc-auto-contouring/hnc_rigs_all/data_nifty/t_l64/predictions_nnunet_repeat/labels_nn_unet_ensemble_final'
    ...
    case_list_pred_r = listdir(label_niis_pred_r)
    case_list_pred_r = [case for case in case_list_pred_r if case.endswith(('.nii.gz'))]
    case_list_pred_r = [label_niis_pred_r + '/' + case for case in case_list_pred_r]
    ...
    biomarkers_pred_r = pool.map(get_biomarkers, case_list_pred_r)
    ...
    for element in biomarkers_pred_r:
        biomarker += element[0]
        value += element[1]
        method += ['dl_r'] * 6
        case_list += element[2]
    ...
    d_biomarkers.to_csv(join(result_dest, 'biomarkers_l64_dl_repeat_v1.csv'),
                     index=False)
...
def generate_metrics_for_method(case):
    print(case)
    path_to_case_gt = join(path_folder_nn_unet, basename(case))
...
if __name__ == '__main__':
    predicted_nn_unet_r = ('/homes/kovacs/project_data/hnc-auto-contouring/'
                           'hnc_rigs_all/data_nifty/t_l64/predictions_nnunet_repeat'
                           '/labels_nn_unet_ensemble_final')

    path_folder_nn_unet = ('/homes/kovacs/project_data/hnc-auto-contouring/'
    ...
    list_of_cases_nn_unet_r = listdir(predicted_nn_unet_r)
    list_of_cases_nn_unet_r = [case for case in list_of_cases_nn_unet_r if
                               case.endswith(('.nii.gz'))]
    list_of_cases_nn_unet_r = [predicted_nn_unet_r + '/' + x for x in
                               list_of_cases_nn_unet_r]

    pool = Pool()
    case_data_all_nn_unet_r = pool.map(generate_metrics_for_method,
                                       list_of_cases_nn_unet_r)

    for case in case_data_all_nn_unet_r:
        metric += case[0]
        value += case[1]
        method += ['nnUNet'] * 8
        case_list += case[2]
    ...
    d_metrics.to_csv(join(result_dest, 'metrics_test64_dl_r.csv'), index=False)

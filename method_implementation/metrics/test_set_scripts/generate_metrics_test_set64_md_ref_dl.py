...
from MEDIcaTe.file_folder_ops import basename, join, listdir, isfile
import pandas as pd
...
def generate_metrics_for_method(case):
    ...
    if (isfile(path_to_case_gt) & isfile(path_to_case_pred)):
...
if __name__ == '__main__':
    ...
    predicted_md = ('/homes/kovacs/project_data/hnc-auto-contouring/hnc_rigs_all/data_nifty/t_l64/labels/labels_doctor_study/md_renamed')

    path_folder_ground_truth = ('/homes/kovacs/project_data/hnc-auto-contouring/hnc_rigs_all/data_nifty/t_l64/labels/labels_doctor_study/ref_renamed')
    ...
    list_of_cases_md =  listdir(predicted_md) # ['HNC03_025.nii.gz']
    list_of_cases_md = [predicted_md + '/' + x for x in
                        list_of_cases_md]

    list_of_cases_nn_unet = listdir(predicted_nn_unet) # ['HNC03_025.nii.gz'] # 
    ...
    case_data_all_md = pool.map(generate_metrics_for_method,
                                list_of_cases_md)
    ...
    d_metrics.to_csv(join(result_dest, 'metrics_test64_clinical_study_v3.csv'),
                     index=False)
